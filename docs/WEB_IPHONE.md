...
> **重要:** Google 等で「まもる計算」を検索しても**出てきません**。  
> Safari の**アドレスバー**に URL を直接入力してください（検索ボックスではありません）。

### 公開 URL（GitHub Pages）

詳細手順 → **[GITHUB_PAGES.md](./GITHUB_PAGES.md)**
...
※ 初回は GitHub の Settings → Pages → Source を **GitHub Actions** に設定してください。

### ローカル URL（Mac のサーバー起動時のみ）
...
| 真っ白な画面 | 修正済み（2026-06-25）— ページを再読み込み。Mac で `./scripts/serve_web.sh` を再起動 |
